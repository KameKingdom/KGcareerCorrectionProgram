        ...
        with open(FILE_DIRECTORY + "newFormatTemp.csv", mode='w', encoding='cp932', newline="") as write_csv:
            ...
            for read_csv_row in reader:
                ...
                if new_student_number != old_student_number:
                    ...
                    converted_user_answer = read_csv_row["ユーザーの回答内容"]
                    converted_user_answer = re.sub(
                        r'[\n\s\"\\\t]', '', converted_user_answer)
                    student_answer_info = [
                        read_csv_row['提出日時'], read_csv_row['提出レポートID'], read_csv_row['名前（姓）'], converted_user_answer]
                else:
                    converted_user_answer = read_csv_row["ユーザーの回答内容"]
                    converted_user_answer = re.sub(
                        r'[\n\s\"\\\t]', '', converted_user_answer)
...
for i in range(COLUMN_NUM):
    ...
    if i == interval:
        ...
        finish_time = datetime.datetime.now() + datetime.timedelta(minutes=minute, seconds=second)  # 加える時間をtimedeltaで定義
        print(f"\n\n\n\n【終了予定時刻】")
